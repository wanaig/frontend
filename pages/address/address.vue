	...
	<view class="container">
		<view class="main">
			<view v-if="loading" class="no-address-tips">
				<image src="/static/images/loadinggg.gif" style="width: 100rpx; height: 100rpx;"></image>
			</view>
				...
				<uni-swipe-action>
					<uni-swipe-action-item class="address-wrapper" :options="swipeOption" @click="handleSwipeClick(address.id)" v-for="(address, index) in addresses" :key="index">
						<view class="address" @tap="chooseAddress(address)">
							...
							<image src="/static/images/edit.png" class="edit-icon" @tap.stop="edit(address)"></image>
						</view>
...
<script>
	...
	import {deleteApi, getAddressList} from '@/api/index.js'
	...
	export default {
		...
		methods: {
			...
			edit(address) {
				...
				uni.navigateTo({ url: '/pages/address/add?id=' + address.id })
			},
			handleSwipeClick(id) {
				uni.showModal({
					title: '提示',
