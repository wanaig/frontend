		...
		<view class="main" v-if="goods.length">
			...
			<view class="content">
				<scroll-view class="menus" :scroll-into-view="menuScrollIntoView" scroll-with-animation scroll-y>
					<view class="wrapper">
						<view class="menu" :id="`menu-${item.id}`" :class="{'current': item.id === currentCateId}" v-for="(item, index) in categories"
				...
				<!-- goods list begin -->
				<scroll-view class="goods" scroll-with-animation scroll-y :scroll-into-view="goodsScrollIntoView" @scroll="handleGoodsScroll">
					<view class="wrapper">
...
import {getStore, getGoodsCategory} from '@/api/index.js'
	...
	methods: {
		...
		async init() {	//页面初始化
			...
			try {
				...
				console.log('[门店] 获取成功:', store)

				// 获取分类列表
				this.categories = await getGoodsCategory()
				console.log('[分类] 获取成功:', this.categories)
		...
		/**
		 * loadAllGoods - 加载所有分类商品（长列表模式）
		 */
		async loadAllGoods() {
			try {
				console.log('[商品] 正在加载所有分类...')
				const data = await this.$api('goods')
				console.log('[商品] 获取成功:', data)

				// 转换后端数据格式到前端格式
				if (Array.isArray(data)) {
					this.goods = data.map(cate => ({
						id: cate.id,
						name: cate.name,
						sort: cate.sort,
						icon: cate.icon || '',
						goods_list: (cate.goodsList || []).map(good => ({
							id: good.id,
							name: good.name,
							price: good.price,
							images: good.images || '',
							content: good.content || '',
							use_property: good.useProperty ? 1 : 0,
							property: good.properties || []
						}))
					}))
				} else {
					this.goods = []
				}
				console.log('[商品] 转换后数据:', this.goods)
				...
				console.error('[商品] 获取失败:', err)
				this.goods = []
			}
		},

		/**
		 * loadGoodsByCategory - 根据分类ID加载单个分类商品
		 * @param {Number} categoryId - 分类ID
		 */
		async loadGoodsByCategory(categoryId) {
			try {
				console.log('[商品] 正在加载分类:', categoryId)
				const data = await this.$api('goods', { category_id: categoryId })
				console.log('[商品] 获取成功:', data)
		...
		handleMenuTap(id) {	//点击菜单项事件
			...
			this.goodsScrollIntoView = 'cate-' + id
			this.menuScrollIntoView = 'menu-' + id
		},
